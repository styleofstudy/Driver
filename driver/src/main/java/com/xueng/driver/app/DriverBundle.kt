...
class DriverBundle  private constructor(val bundle: Bundle){

    /** 将map中的所有数据均存放至容器中*/
    fun put(map:Map<String, Any?>):DriverBundle {
        map.forEach { put(it.key, it.value) }
        return this
    }

    /** 直接一起存储不定数量的键值对数据到容器中*/
    fun put(vararg items:Pair<String, Any?>):DriverBundle {
     ...
     */
    fun put(key:String, value:Any?):DriverBundle {
    ...
    private fun parseJSON(json:String, type: Type):Any {
        return when {
            GSON -> GsonUtils.fromJson(json, type)
    ...
    companion object {
        ...
        @JvmStatic
        fun create(source:Bundle? = null): DriverBundle {
            return DriverBundle(source?: Bundle())
        }
...
private class BundleInjector {
    ...
    fun toEntity(entity:Any, bundle: Bundle):Any {
        ...
        val easyBundle = DriverBundle.create(bundle)
